...
- **One user action no longer refreshes a change request once per policy.** Attaching a policy is a separate signal for each, so submitting a request governed by three policies recomputed its status and ran every check four times over for an identical answer. Those bursts are now collapsed into a single refresh per change request. Nothing is deferred past the commit: the work still finishes before the view returns.
...
- **Return to draft.** A submitted change request can be pulled back out of review, by its author or anybody holding `change_changerequest`, from the change request page or `POST /change-requests/{id}/return-to-draft/`. It is the reversible counterpart to abandoning: the reviews are kept, and resubmitting picks up where it left off. Allowed from Approved as well, so an author who spots a problem after approval can take the change off the table rather than race the merge; it only ever closes the merge gate, never opens it.
- **Draft is now a state the author holds** rather than one the evaluation computes. A review submitted against a draft moves nothing, and a request pulled back stays pulled back. Without this, **Return to draft** would be undone by the next signal. Submitting is what leaves draft, and it is also what matches the policies and notifies the reviewers.
- `submit` is a REST action too, so an integration can drive the whole lifecycle now that `status` is read-only. [The lifecycle diagram](docs/change-requests.md#the-lifecycle) shows every state and transition, which is automatic and which is somebody pressing a button.
...
- **Reopening an abandoned request now returns it to Draft** rather than pushing it straight back into review. Its reviews may have gone stale and its policies may have moved while it was set aside, so it goes back to its author, who submits it again when it is ready.
- **Submit for review** moved to the page's control bar, joining Abandon, Reopen and Return to draft. All four are actions on the whole request, and the Applied policies card now carries only the policies.
